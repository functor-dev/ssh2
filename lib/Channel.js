...
function Channel(info, conn) {
  ...
  */
  ...
  conn._parser.on('CHANNEL_DATA:' + this.incoming.id, function(data) {
    ...
    } else if (self._callbacks.length
               && (!self._dataBuffer
                   || self._dataBuffer.length < MAX_BUFFER_BEFORE_CB)) {
      // this is a workaround for some servers that start sending channel data
      // before the channel success message. here we buffer at most
      // MAX_BUFFER_BEFORE_CB bytes before the channel success message. if we
      // see more than that, we'll just drop the extra data for now to prevent
      // buffering way too much ...
      if (!self._dataBuffer)
        self._dataBuffer = data;
      else {
        self._dataBuffer = Buffer.concat([self._dataBuffer, data],
                                         self._dataBuffer.length + data.length);
      }
    }
  ...
  conn._parser.on('CHANNEL_SUCCESS:' + this.incoming.id, function() {
    if (self._callbacks.length) {
      ...
      if (self._stream && self._dataBuffer) {
...
Channel.prototype._sendAgentFwd = function(cb) {
  if (this.outgoing.state !== 'open') {
    cb(new Error('Channel no longer open'));
    return true;
  }
